...
function addTorrent(url:string) {
...
function getTorrentDetails(id:number) {
...
function deleteTorrent(id: number) {
  transmission.remove(id, true, function (err, result) {
    if (err) {
      ...
      result.torrents.forEach(function (torrent: any) {
...
function startTorrent(id: number) {
...
function stopTorrent(id: number) {
...
// Get torrent state 
function getStatusType(type: number) {
  if (type === 0) {
    return 'STOPPED';
  } else if (type === 1) {
    return 'CHECK_WAIT';
